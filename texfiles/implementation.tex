...
actual implementation purposes is handeled by having a table in between the two
tables, that link them together with a one-to-many relation to both of the
tables. 

Useually you would use the foreign keys as a composit key so there cannot be any
duplicate connection.

Because this can easily create multiple cascade paths on delete, I have desided
...
alwayse manually create the many-to-many relation. This is because entity
framework can figure that out itself, asuming i give it the information it
...
\todo{Need a few more integration tests}
\begin{figure}[h]
  \centering
...
The unit tests are generally more usefull for the development, in that they help
...
is a powerful list comprehention library in C\#.
...
save when the controller desides that it should be done. To make sure that it is
obvious that is the way to do it, I have created a unit of work class, where the
only thing it can do is call save. This way I can also ignore what kind ov ORM I
am using for communicating with the datastorage entirely in the controllers, as
...
NotFoundException, which will propergate all the way outside the controller. The
...
a spesific thing. Under the development I created unit tests, so that I could
see if a single bit of the program dod what i expected it it. For the unit
...
Because there are more smaller units in the application than there are usecases
there are a lot more unit tests than integrations tests. This is also because
